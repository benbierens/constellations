...
export class Column {
  ...
  processPacket = async (packet) => {
    if (!this._handler)
      this._logger.errorAndThrow("processPacket: handler not set.");
    ...
    if (!packet.header)
      this._logger.errorAndThrow("processPacket: packet has no header.");
    if (!packet.signature)
      this._logger.errorAndThrow("processPacket: packet has no signature.");
    if (!packet.signedData)
      this._logger.errorAndThrow("processPacket: packet has no signedData.");
  ...
  _getAsPacket = () => {
    if (!this._signature)
      this._logger.assert("getAsPacket: signature not set.");
  ...
  _handleRequestPacket = async () => {
    if (this.isReady) {
      ...
      this._logger.trace(
        "_handleRequestPacket: Unable to send response: Data not available.",
      );
    }
  };
  ...
  _processPacket = async (packet, hash, signer) => {
    ...
    const checkResponse = await this._handler.checkUpdate(
      signer,
      packet.signedData.payload,
    );
    if (!checkResponse || checkResponse == ColumnUpdateCheckResponse.Unknown) {
      this._logger.errorAndThrow(
        "_processPacket: Invalid response from columnHandler.checkUpdate.",
      );
    }
    if (checkResponse == ColumnUpdateCheckResponse.Discard) {
      this._logger.trace(
        "_processPacket: Rejected by columnHandler.checkUpdate.",
      );
      return;
    }
    ...
    if (checkResponse != ColumnUpdateCheckResponse.Accept) {
      this._logger.assert(
        `_processPacket: Unknown CheckResponse received from handler: '${checkResponse}'`,
      );
    }
  ...
  _applyUpdate = async (packet, hash, signer) => {
    ...
    await this._handler.onValueChanged(wasReady);
  };
  ...
  _updateDelayedPacket = (packet, hash, signer) => {
    if (
      ...
      packet.signedData.utc >
        this._delayedPacketAndHashAndSigner.packet.signedData.utc
    ) {
      this._delayedPacketAndHashAndSigner = {
        packet: packet,
        hash: hash,
